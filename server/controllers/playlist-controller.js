                ...
                if (!playlists) {
                    ...
                    for (let key in playlists) {
                        ...
                        let pair = {
                            ...
                            numberOfListens: list.numberOfListens,
        ...
        async function asyncFindUser(list) {
            await User.findOne({ email: list.ownerEmail }, (err, user) => {
                ...
                if (user || user == null) {
                    ...
                    list.numberOfListens = body.playlist.numberOfListens;
...
getUserPlaylistsBySearch = async (req, res) => {
    console.log(req.params);
    const searchText = req.params.searchText;
    await Playlist.find({publishedBy: {$regex: searchText}, published: true}, (err, playlists) => {
        if (err) {
            return res.status(400).json({ success: false, error: err })
        }
        if (!playlists.length) {
